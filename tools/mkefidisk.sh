...
echo "Inform the operating system about partition table changes and waiting system recognized "
...
#	mkfs.vfat -I $BOOTFS -n "EFI" >$OUT 2>&1 || die "Failed to format vfat 1 $BOOTFS"
	mkfs.vfat -I /dev/sdb1 -n "EFI" >$OUT 2>&1 || die "Failed to format vfat 1 $BOOTFS"


else
#	mkfs.vfat $BOOTFS -n "EFI" >$OUT 2>&1 || die "Failed to format vfat 2 $BOOTFS"
	mkfs.vfat /dev/sdb1 -n "EFI" >$OUT 2>&1 || die "Failed to format vfat 2 $BOOTFS"
fi

debug "Formatting $ROOTFS as ext3"
mkfs.ext3 -F $ROOTFS -L "ROOT" >$OUT 2>&1 || die "Failed to format ext3 $ROOTFS"
